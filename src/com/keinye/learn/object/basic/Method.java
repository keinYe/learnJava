...
/**
 * private 修饰符限制成员仅能在类内部访问，其它类不能够调用被 private 修饰的内部成员。
 * 如果没有任何修饰符时，默认是 public。
 * 使用 private 修饰的内部字段，可以通过 setter 或 getter 来访问该字段。
 * 类方法通过 实例变量.方法名(参数) 的形式来调用。
 * 在方法内部，有一个隐含的 this 变量，它始终指向当前实例。
 * 方法可以有任意个参数，调用方法时必须按照参数的顺序一一传递。
 * 可变参数 类型... 在方法内部相当于一个数组，可以按照数组的方法来使用。
